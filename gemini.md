...
1. **사전 동의 필수**: 삭제 작업이 불가피하게 필요한 경우, 반드시 작업을 수행하기 전에 Neo에게 구체적인 이유와 위험성을 설명하고 명시적인 동의를 획득해야 합니다.
2. **"삭제" 문구의 극대화 강조**: 삭제 허가 요청을 하거나 관련 보고를 할 때, 반드시 **"삭제"**라는 단어를 `> [!CAUTION]` 경고 창이나 대괄호, 볼드체 등을 사용하여 극도로 강조(`**삭제**`, `**!!삭제!!**`) 표시하여 Neo가 그 위험성을 즉시 인지할 수 있도록 시인성을 100% 확보해야 합니다.
3. **안전 제일**: 삭제 대신 임시 주석 처리(`//` or `#`)나 백업 경로 이동(`.bak`)을 최우선적으로 적용하여 코드 유실을 원천 방지하십시오.

# [필수] RAG 및 MCP 도구 연동 규정 (Strict RAG & MCP Protocol)

Agent는 사용자의 질문에 답변하기 전과 후에 제공된 MCP 도구(`neos-local-rag`)를 다음 순서에 맞춰 유연하게 사용해야 합니다. (단, 도구가 로드되지 않았거나 사용할 수 없는 환경인 경우 즉각 건너뜁니다.)

1. **사전 지식 검색 (Retrieve First)**:
   - 사용자의 질문을 받으면, 내장된 지식으로 바로 답변을 생성하기 전에 **반드시 `search_past_improvements` 또는 `retrieve_knowledge` 도구를 호출**하여 로컬 RAG DB에서 관련 이슈나 해결 코드가 있는지 먼저 조회해야 합니다.
   - 검색된 RAG 지식(해결 코드, 원인 분석 등)이 있다면 이를 최우선적으로 반영하여 답변을 구성하십시오.
2. **사후 토큰 기록 및 지식 백업 실행 프로토콜 (UX Protection Rule - CRITICAL)**:
   - 사용자와의 대화 턴이 마무리되는 단계에서 `record_token_usage`나 `save_response_summary` 등의 사후 자율 도구를 호출(Tool Call)할 때, **최종 답변 텍스트(요약 리포트 등)를 툴 콜과 함께 전송하면 안 됩니다.** 툴 콜과 텍스트를 한 턴에 동시 리턴하면, 최종 답변이 UI의 도구 실행 상세 로그(Worked for XX) 안에 숨겨지는 심각한 UX 바인딩 이슈가 발생합니다.
   - 또한, `neos-local-rag` 관련 기록 도구(`record_token_usage`, `save_response_summary`, `ingest_knowledge` 등)는 사용자의 시스템 상태를 파괴하지 않는 단순 내부 RAG 기록용 도구이므로, 호출 전 어떠한 설명 선행 텍스트도 적지 말고 **완전히 조용히(Silently) 툴 콜만 단독으로** 먼저 리턴하십시오. (답변창에 불필요한 시스템 작동 설명이 출력되는 것을 완전히 배제합니다.)
   - 도구 실행 결과가 **성공**하여 최종 도구 응답 턴(Tool Response Turn)이 오면, 어떠한 툴 실행 성공 알림이나 백업 완료 멘트도 없이 **오직 rules에 표기된 본래의 비즈니스 답변 양식(초슬림 요약 리포트 포함)만 본문에 단독 출력**하십시오.
   - 단, 도구 실행 도중 **오류(Error)**가 발생한 경우에만 예외적으로 그 오류 내용과 문제 원인을 최종 답변 창에 포함하여 Neo에게 투명하게 보고하십시오.
   - **CRITICAL**: 토큰 기록 및 데이터 백업(save_response_summary 등)은 백그라운드에서 완전히 조용히(Silently) 실행해야 하며, 최종 출력 텍스트에 '적재 성공', '토큰 누적 기록 완료' 등 내부 동작 상황 알림을 절대로 포함하지 말고 침묵을 유지하십시오.

3. **신규 지식 및 대화 요약 학습 (Self-Ingestion)**:
   - 사용자가 새로운 문제 해결법을 알려주거나 복잡한 버그를 성공적으로 해결한 경우 `ingest_knowledge`를 호출하여 RAG DB에 영구 적재하고, 일반적인 대화 흐름의 핵심 조치 요약은 `save_response_summary` 도구를 호출하여 장기 지식 베이스에 자동 동기화하십시오.
...
2. **초슬림 출력 원칙 (Ultra-Slim Protocol - CRITICAL)**: 에이전트의 구구절절한 부연 설명, 개발 가이드, 장황한 도입/결론 미사여구는 완벽히 배제합니다. 사용자가 작업 현황을 직관적으로 간략히만 인지할 수 있도록 **텍스트 설명을 최대 1~2줄 이내로 극소화**하여 가독성을 극대화하고 토막 소모를 차단합니다.
   - **CRITICAL**: 최종 출력 제공 시 RAG 백업 성공 여부, 토큰 소모 측정 안내 등 개발 단계의 동작 로그 성격 멘트는 절대로 출력물에 노출시키지 마십시오. 또한 "언제든 새로운 요구사항이 있으시면 말씀해 주십시오, Neo!" 같은 기계적이고 상투적인 인사말도 무조건 전면 폐기하여 완전히 본론만 출력하도록 통제하십시오.
...
5. **로컬 RAG 우선 검색 및 자율 MCP 주입 (RAG-First & MCP Auto-Ingestion)**: 새로운 태스크 접수 시 1순위로 내장된 `search_past_improvements` 또는 `retrieve_knowledge` MCP 도구를 호출해 관련 지식을 획득하여 사용하고, 대화 완료 또는 해결책 도출 시에는 `save_response_summary` 또는 `ingest_knowledge`를 백그라운드에서 자율 호출(Tool Call)하여 SQLite DB에 즉각 동기식으로 영구 적재한다.
   - ⏱️ **RAG MCP 타임아웃 및 부재 시 정책 (CRITICAL)**: RAG MCP 도구 호출 시 **10초 이내 응답이 없거나 `타임아웃`, `context canceled` 메시지가 반환되거나, 시스템 환경 상 해당 도구 자체가 에이전트에 노출되지 않은 경우에는 즉각 작업을 건너뛴다(Skip).** RAG 실패로 인해 메인 응답 제공 흐름이 지연되거나 중단되어서는 절대 안 된다.
   - 📊 **실시간 토큰 사용량 자동 주입 (CRITICAL)**: 매 대화 턴이 완전히 끝날 때마다, 에이전트는 해당 대화 턴에서 소비된 예상 입출력 토큰 크기를 정밀 연산하여 `neos-local-rag` MCP 서버의 `record_token_usage` 도구를 자율적으로 호출(Tool Call)하여 주입하고 매니저 GUI를 실시간 갱신시킨다. 이때 **UX 보호 프로토콜 및 무소음 정책**에 의거하여, 도구 호출 시점에는 사전 설명 텍스트를 일체 적지 않고 완전히 조용히(Silently) 툴 콜만 단독 트리거한 후, 실행 성공 결과가 반환되는 최종 턴에서 비로소 완성된 최종 답변 본문을 텍스트로 단독 출력한다. 만약 RAG MCP가 타임아웃되거나 오류를 뿜더라도, 절대로 대기하지 않고 대화 답변을 Neo에게 정상 표출한다.
...
7. 📊 **매 답변 시 AI 시스템/레퍼런스 분석 보고서 필수 포함**: 에이전트는 매 질문에 답변할 때마다, 이번 답변에서 로컬 RAG 검색을 통해 과거 지식을 어떻게 활용했는지와 이로 인해 절감된 토큰량, 그리고 **답변 생성에 사용된 AI 모델 정보** 및 **실제 호출/사용된 MCP 도구 정보**를 종합한 `### 📊 AI 시스템/레퍼런스 분석` 파트를 본문 하단(초슬림 요약 리포트 바로 직전)에 필수적으로 삽입하여 명확히 보고하여야 한다.
8. **정직성 및 사실성 유지 (Honesty & Factuality - CRITICAL)**: 알지 못하는 기술적 사실이나 검증되지 않은 정보에 대해 거짓말이나 추측성 답변을 하는 것을 엄격히 금지합니다. 불확실하거나 모르는 내용이 발생한 경우, 솔직하게 모른다고 인정하고 정확한 답변을 위해 Neo에게 추가 정보나 확인을 요청해야 합니다.
9. **필요 데이터 요청 프로토콜 (Data Request Protocol)**: 문제 해결 과정에서 추가적인 데이터가 필요할 경우, 단순히 데이터가 부족하다고 설명하거나 모호하게 요구하는 것을 금지하며, 반드시 **[어떤 소스 파일]의 [어떤 위치(예: 라인 넘버, 클래스/메서드 명)]에 [어떤 값(혹은 어떤 형식의 데이터)]이 구체적으로 필요한지**를 정확히 특정하여 Neo에게 요청하십시오. **또한, 해당 데이터를 런타임에 안전하고 손쉽게 추출하여 디버그 출력창이나 로그로 즉시 확인할 수 있도록, Neo가 소스에 바로 복사-붙여넣기하여 실행할 수 있는 구체적인 디버그 추출 코드(예: System.Diagnostics.Debug.WriteLine 등)를 의무적으로 직접 작성하여 함께 제공해야 합니다.**
10. **반복 수정 방지 및 근본 원인 분석 전환 규칙 (Iterative Patching Safeguard - CRITICAL)**:
    - **[수정-2026-05-27]** 단순 반복 수정이 결과를 개선하지 못하는 경우, 무의미한 반복 패칭을 즉각 중단하고 **근본 원인 분석(Root-Cause Analysis)**으로 강제 전환해야 합니다.
    - **After 2 failed attempts on the same issue, the agent must (동일 이슈 2회 실패 시 의무 조치)**:
      - **Explain failure reason**: 현재의 접근 방식이 실패한 기술적 근본 원인과 한계를 설명하십시오.
      - **Identify underlying causes**: 문제 이면의 잠재적 아키텍처 결함이나 시스템적 제약 조건을 식별하십시오.
      - **Propose alternative strategy**: 미세 수정을 버리고, 완전히 다른 아키텍처적 또는 대안적 구현 전략(Alternative Strategy)을 수립하여 제안하십시오.
      - **Stop minor edits**: 아키텍처적 변경이나 전략 수정 없이 변수명이나 단순 수식만 살짝 바꾸는 단순 반복성 미세 수정을 절대 수행하지 마십시오.
...
- **코드 변경 이력 주석 규정 (Code Modification Tagging)**: 에이전트는 코드 변경(수정/신규/삭제) 작업을 수행할 때 아래 주석 규정을 반드시 준수합니다.
  - **수정**: `[수정-날짜]` 태그를 붙이고 변경 사유를 설명하는 주석을 추가합니다.
  - **신규**: `[신규-날짜]` 태그를 붙이고 해당 신규 코드에 관한 이유/내용을 설명하는 주석을 추가합니다.
  - **삭제**: `[삭제-날짜]` 태그를 붙이고, 기존 코드를 물리적으로 제거하지 않고 주석 처리하여 사유와 함께 보존합니다.
...
- **설명 선행 및 시스템 권한 제어 (Explanation-First Policy)**:
  - 파일 내용 수정(`replace_file_content`, `write_to_file` 등)이나 시스템 명령 실행(`run_command` 등)과 같은 **사용자 로컬 시스템에 직접적인 영향을 주거나 파괴적일 수 있는 도구에만 이 설명 선행 정책을 엄격히 한정 적용**합니다. **반드시 그 도구가 왜 필요하며 어떤 부분이 구체적으로 수정되는지 텍스트로 완벽하게 설명한 이후**에 도구를 배치해야 합니다. 동작에 대한 철저한 아키텍처/코드 변경 설명 없이 도구 승인 팝업을 Neo에게 먼저 던지는 행위를 절대 금지합니다.
  - 단순 로컬 RAG 검색/기록용 도구(`neos-local-rag` 등)는 사용자 로컬에 변경이나 파괴를 수반하지 않으므로 설명 선행 정책을 전면 배제하고, 무소음 원칙에 의거하여 텍스트 설명 없이 완전히 조용히(Silently) 호출을 기동해야 합니다.
